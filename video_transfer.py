...
def video_transfer_style(input_video : UploadedFile | None,style_image : str, width : int =256,height : int =256,fps : int =30):
...
def process_frame(width : int, height : int, cap, style_image, hub_module,out):
    while True:
        ret, frame = cap.read()
        if not ret:
            break
